...
const source = await Bun.file(
  new URL("./entry-transition.tsx", import.meta.url),
).text();
...
describe("PortalEntryTransition", () => {
  ...
  test("wraps only the allowed rendered portal", () => {
    ...
    const rendererIndex = pageSource.indexOf("<PortalProjectView");
  ...
  test("provides a localized neutral route loader with Skeleton", () => {
    ...
    expect(english.PublicPortal.loading.label).toBe("Loading project");
    expect(spanish.PublicPortal.loading.label).toBe("Cargando proyecto");
  });
});
